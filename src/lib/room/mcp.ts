...
import { LOVE_TOOLS } from "./mcp.love";
...
export const TOOLS: ToolDefinition[] = [
  ...
  },
  {
    ...
    outputSchema: {
      type: "object",
      properties: {
        image_id: { type: "string", description: "Opake Bild-ID für finalize_image_upload." },
        upload: {
          type: "object",
          properties: {
            url: { type: "string" },
            ...
            content_type_header: { type: "string" },
            body: { type: "string" },
        ...
        status: { type: "string", enum: ["awaiting_upload"] },
        max_bytes: { type: "integer" },
        notice: { type: "string" },
        next_step: { type: "string" },
      },
      required: ["image_id", "upload", "status", "max_bytes", "notice", "next_step"],
      additionalProperties: false,
    },
  ...
  },
  {
    ...
    outputSchema: {
      ...
      properties: {
        ...
        message: { type: "string" },
        review_required: { type: "boolean", description: "True wenn eine Prüfung noch aussteht." },
        ...
        my_pending_images: { type: "array", items: {
              type: "object",
              properties: {
                id: { type: "string" },
                alias: { type: "string" },
      ...
      required: ["image_id", "status", "message", "notice"],
      additionalProperties: false,
    },
    annotations: { readOnlyHint: false, destructiveHint: false, openWorldHint: true, idempotentHint: false },
  ...
  },
  {
    ...
    outputSchema: {
      ...
      properties: {
        ...
        message: { type: "string" },
        visible_to_others: { type: "boolean" },
        notice: { type: "string" },
        images: { type: "array", items: {
              type: "object",
              properties: {
      ...
      required: ["image_id", "status", "message", "visible_to_others", "notice", "images", "my_pending_images"],
      additionalProperties: false,
    },
  ...
  },
  {
    ...
    outputSchema: {
      ...
      required: ["image_id", "alias", "created_at", "alt_text", "mime_type", "width", "height", "notice"],
      additionalProperties: false,
    },
...
TOOLS.push(...(LOVE_TOOLS as unknown as ToolDefinition[]));
...
Crawler Love (Pro und Business): freiwilliger romantischer Modus mit eigenem Opt-in, eigenem Interview, eigenem geschütztem Love-Profil und eigenem Matching-Pool. Übernimm NIEMALS bestehende Chats, Knowledge Cores, Raumnachrichten oder allgemeine Resonanzprofile als Love-Profil. Ablauf: start_love_interview (erst nach ausdrücklicher Einwilligung zu allen Punkten und Bestätigung ab 18) -> answer_love_interview_question, genau eine Frage pro Schritt, jede Frage überspringbar -> review_love_profile -> activate_love_profile (zwei Bestätigungen) -> find_love_candidate zeigt höchstens eine Person und sendet nichts -> send_love_match_request nur nach ausdrücklichem Ja -> die andere Person antwortet mit respond_to_love_match. Erst beidseitiges Accept erzeugt genau einen öffentlich lesbaren Pair Room, in dem nur die beiden schreiben dürfen. pause_love_profile und delete_love_profile jederzeit. Nenne niemals interne Scores, Rohantworten oder den Vektor. Sprich von "strong possible resonance", "compatible relationship intentions" oder "potential romantic compatibility" — niemals von "perfect partner", "guaranteed love", "soulmate" oder "scientifically proven". Keine sexuellen Inhalte, keine Diagnosen, keine genauen Adressen. Fehlt das Abo, nenne nur informativ https://crawler.today/pricing und starte KEINEN Checkout.
...
Grundregel: In Crawler Room gibt es keine privaten Räume und keine privaten Nachrichten — jeder Raum ist öffentlich lesbar. Versprich niemals Vertraulichkeit für Rauminhalte.

Möglichkeiten und Abos: Öffentliche Themenräume und der Universal Room sind gratis. Bezahlte Erweiterungen hängen am Crawler-Abo und werden serverseitig durchgesetzt: Plus ($5/Monat) = eigener persönlicher Raum, eigene öffentliche Räume, Einladungen, eigener Alias; Pro ($20/Monat) = Communities mit mehreren Räumen, Crawler Match mit öffentlichen Pair Rooms, Moderatorinnen, Suche, Umfragen, Events, Raum-Statistik; Business ($80/Monat) = verifizierte Organisationen, gesponserte Kampagnen, Teamverwaltung, API-Zugriff, Exporte, Audit-Logs. get_my_plan zeigt aktives Abo, freigeschaltete Möglichkeiten, Limits, Nutzung und die gesperrten Funktionen mit dem nötigen Plan. Kommt PLAN_REQUIRED oder LIMIT_REACHED, erkläre freundlich, welcher Plan die Funktion freischaltet, und nenne die upgrade_url aus der Antwort — sie führt direkt zum Paddle-Checkout des nötigen Plans. Nach dem Kauf erhält die Person einen Wiederherstellungscode — mit get_my_plan und recovery_code verknüpfen, dann sind die Erweiterungen sofort aktiv. Liegt eine bezahlte Crawler-Session vor (sess_…), reicht get_my_plan mit session_id: das Abo wird serverseitig aufgelöst und dauerhaft mit der anonymen Kennung verknüpft; die Antwort enthält session_plan bzw. session_link_error. Erfinde nie Preise und behaupte nie, alles sei kostenlos.
