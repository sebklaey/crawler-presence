...
export default defineTool({
  ...
  outputSchema: {
    ...
    no_own_model: z.boolean().optional().describe("Always true: Crawler runs no language model, the calling assistant interviews."),
    interviewer_instructions: z.string().optional().describe("How the calling assistant should phrase the next question."),
    merge_mode: z.string().optional().describe("structured_update when core_update was sent, otherwise rule_based_fallback."),
    entity_type: z.string().optional(),
    confidence: z.number().optional(),
    interview_complete: z.boolean().optional().describe("True when no critical gap is left — offer publishing."),
    presence_score: z.number().optional().describe("Knowledge Core completeness, 0-100."),
    open_checks: z.array(z.string()).optional(),
    open_gaps: z.array(z.object({ key: z.string().optional(), label: z.string().optional(), suggested_question: z.string().optional() })).optional(),
    next_gap: z.object({ key: z.string().optional(), label: z.string().optional(), suggested_question: z.string().optional() }).nullable().optional(),
    verified_facts: z.array(z.object({ label: z.string().optional(), value: z.string().optional() })).optional(),
    claimed_facts: z.array(z.object({ label: z.string().optional(), value: z.string().optional() })).optional(),
    narrative: z.array(z.object({ label: z.string().optional(), text: z.string().optional(), confirmed: z.boolean().optional() })).optional(),
    ...
    publish_handoff_url: z.string().nullable().optional(),
    publish_prompt: z.string().nullable().optional(),
    publish_instructions: z.string().nullable().optional(),
  },
