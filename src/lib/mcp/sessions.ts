...
export const SESSION_NOTE =
  "Anonymous draft session: identified only by an opaque random token, stored durably for ~30 days. Crawler has no user accounts, no login and no registration, so a draft is never linked to a person. Anyone holding the token can open the draft, so treat it as a shareable link. Publishing is the paid step and hands out a one-time management secret that controls the published Presence.";
