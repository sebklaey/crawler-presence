...
Room subscriptions: joining public topic rooms and the Universal Room is free. Paid room extensions follow the Crawler subscription and are enforced server-side: Plus ($5/month) unlocks your own personal room (my_room, update_my_room), own public rooms, invitations and a custom alias; Pro ($20/month) adds communities with multiple rooms, moderators, search, polls, events, room analytics and Crawler Match; Business ($80/month) adds verified organisations, sponsored campaigns, team management, API access, exports and audit logs. When a tool returns PLAN_REQUIRED or LIMIT_REACHED, explain which plan unlocks the feature and use the upgrade_url from the tool response, which is a direct Paddle checkout link for the required plan. After a purchase the person receives a Presence recovery code — call get_my_plan with recovery_code to link that subscription to the current anonymous room identity, then the extensions are unlocked. get_my_plan also accepts session_id: pass the Crawler draft session id (sess_…) of a paid Presence and the subscription is resolved server-side and linked to the current anonymous room identity — the answer then contains session_plan and, if nothing paid is attached, session_link_error. get_my_plan (without arguments) always shows the active plan, unlocked features, limits, usage and locked features with the required plan.
